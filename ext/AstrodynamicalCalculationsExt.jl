...
AC.inclination(orbit::R2BOrbit) = AC.inclination(orbit.state, orbit.parameters)
...
AC.true_anomaly(state::CartesianState, μ) = AC.true_anomaly(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.true_anomaly(orbit::R2BOrbit) = AC.true_anomaly(orbit.state, orbit.parameters)
...
AC.semi_parameter(state::CartesianState, μ) = AC.semi_parameter(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.semi_parameter(orbit::R2BOrbit) = AC.semi_parameter(orbit.state, orbit.parameters)

AC.specific_energy(state::KeplerianState, μ) = AC.specific_energy(state.a, gm(μ))
AC.specific_energy(state::CartesianState, μ) = AC.specific_energy(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.specific_energy(orbit::R2BOrbit) = AC.specific_energy(orbit.state, orbit.parameters)

AC.specific_potential_energy(state::KeplerianState) = AC.orbital_radius(semi_parameter(state.a, state.e), state.e, state.ν)
AC.specific_potential_energy(state::KeplerianState, μ) = AC.specific_potential_energy(state)
AC.specific_potential_energy(state::CartesianState, μ) = AC.specific_potential_energy(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.specific_potential_energy(orbit::R2BOrbit) = AC.specific_potential_energy(orbit.state, orbit.parameters)

AC.specific_angular_momentum(state::CartesianState, μ) = AC.specific_angular_momentum(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.specific_angular_momentum(state::KeplerianState, μ) = AC.specific_angular_momentum(keplerian_to_cartesian(state, μ), μ)
AC.specific_angular_momentum(orbit::R2BOrbit) = AC.specific_angular_momentum(orbit.state, orbit.parameters)

AC.specific_angular_momentum_vector(state::CartesianState, μ) = AC.specific_angular_momentum_vector(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.specific_angular_momentum_vector(state::KeplerianState, μ) = AC.specific_angular_momentum_vector(keplerian_to_cartesian(state, μ), μ)
AC.specific_angular_momentum_vector(orbit::R2BOrbit) = AC.specific_angular_momentum_vector(orbit.state, orbit.parameters)

AC.c3(state::CartesianState, μ) = AC.c3(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.c3(state::KeplerianState, μ) = AC.c3(keplerian_to_cartesian(state, μ), μ)
AC.c3(orbit::R2BOrbit) = AC.c3(orbit.state, orbit.parameters)

AC.v_infinity(state::CartesianState, μ) = AC.v_infinity(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.v_infinity(state::KeplerianState, μ) = AC.v_infinity(keplerian_to_cartesian(state, μ), μ)
AC.v_infinity(orbit::R2BOrbit) = AC.v_infinity(orbit.state, orbit.parameters)

AC.orbital_period(state::KeplerianState, μ) = AC.orbital_period(state.a, gm(μ))
AC.orbital_period(state::CartesianState, μ) = AC.orbital_period(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.orbital_period(orbit::R2BOrbit) = AC.orbital_period(orbit.state, orbit.parameters)

AC.orbital_radius(state::KeplerianState, μ) = AC.orbital_radius(AC.semi_parameter(state.a, state.e), state.e, state.ν)
...
AC.mean_motion(state::CartesianState, μ) = AC.mean_motion(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.mean_motion(orbit::R2BOrbit) = AC.mean_motion(orbit.state, orbit.parameters)
...
AC.apoapsis_radius(state::CartesianState, μ) = AC.apoapsis_radius(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.apoapsis_radius(orbit::R2BOrbit) = AC.apoapsis_radius(orbit.state, orbit.parameters)
...
AC.periapsis_radius(state::CartesianState, μ) = AC.periapsis_radius(state.x, state.y, state.z, state.ẋ, state.ẏ, state.ż, gm(μ))
AC.periapsis_radius(orbit::R2BOrbit) = AC.periapsis_radius(orbit.state, orbit.parameters)
...
function Base.show(io::IO, ::MIME"text/plain", orbit::R2BOrbit)
    println(io, "$(AC.conic(orbit)) Orbit in $(AstrodynamicalModels.paradigm(orbit.parameters))\n")
